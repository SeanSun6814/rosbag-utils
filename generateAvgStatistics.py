...
def processOdometry(paths, targetTopic):
    ...
    for path, pathIdx in zip(paths, range(len(paths))):
        ...
        for topic, msg, t in tqdm(bagIn.read_messages(topics=[targetTopic]), total=totalMessages):
            ...
            if first_frame:
                ...
                delta_q = quaternion_multiply(prev_orientation_inv, cur_orientation)
                if delta_q[3] - 1 > 0:
                    delta_q = np.array([0, 0, 0, 1])
                delta_ang = np.arccos(delta_q[3]) * 2

                if np.isnan(delta_ang):
                    print("delta_q:{},{},{},{}".format(delta_q[0], delta_q[1], delta_q[2], delta_q[3]))
    ...
    print("sum_a:{}, sum_angular_rate:{}".format(sum_a, sum_angular_rate))
    print("avg_accelaration:{}, avg_angular_rate:{}".format(avg_accelaration, avg_angular_rate))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--topic_name", default="/aft_mapped_to_init_imu", help="topic name that contains the ground truth odomtry"
    )
    ...
    full_path_list = [os.path.join(args.datapath, x) for x in bags if x.split(".")[-1] == "bag"]
