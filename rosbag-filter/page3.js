...
function updateStep3Finished() {
    if (!document.getElementById("standstillSwitch").checked || cropData.length > 0)
        if (
            !document.getElementById("trajectoryLengthSwitch").checked ||
            document.getElementById("trajectoryTopicSelect").value.trim() !== ""
        )
...
function checkEnableStandstill() {
    ...
    if (checkbox.checked) {
        ...
        document
            .getElementById("standstillTimeInput")
            .parentElement.parentElement.parentElement.classList.remove("disabled");
        ...
        document
            .getElementById("standstillTimeInput")
            .parentElement.parentElement.parentElement.classList.add("disabled");
        cropData = [];
...
function checkTrajectoryLength() {
    ...
    if (checkbox.checked) {
        checkboxLabel.innerHTML = `Enabled`;
        document
            .getElementById("trajectoryTopicSelect")
...
function updateCroppingTable() {
    ...
    for (let idx = 0; idx < files.length; idx++) {
        ...
        cropFrom =
            "<a onclick='onChangeCropData(" +
            idx +
            ",true);' style='color: #ff6200; cursor: pointer;'>" +
            cropFrom +
            "</a>";
        cropTo =
            "<a onclick='onChangeCropData(" +
            idx +
            ",false);' style='color: #ff6200; cursor: pointer;'>" +
            cropTo +
            "</a>";
...
function onChangeCropData(fileIdx, isStart) {
    ...
    openInputBox(
        ...
        (newValue) => {
            newValue = Math.max(
                files[fileIdx].info.start,
                Math.min(parseFloat(newValue) + files[fileIdx].info.start, files[fileIdx].info.end)
            );
...
function updateCroppingData(callback) {
    ...
    if (topicName.trim() === "")
        return showAlert(
            "No topic selected",
            "Select an odometry topic to compute the cropping.",
            "error",
            "Got it",
            () => {}
        );
    if (!standstillSecondsIsValid())
        return showAlert("Standstill seconds invalid", "Enter a valid number.", "error", "Got it", () => {});
